...
then
        subject="ADDM and AWR Reports for ${COD}${hulaTail}: ${startSnapTime} to ${endSnapTime} (UTC)"
...
then
        subject="AWR Reports for ${COD}${hulaTail}: ${startSnapTime} to ${endSnapTime} (UTC)"
...
then
        subject="ADDM Reports for ${COD}${hulaTail}: ${startSnapTime} to ${endSnapTime} (UTC)"
...
For troubleshooting, pelase contact Jesus.Sanchez.DBA@wellsfargo.com</br>
"
...
Subject: ${subject}
Date: $(date +%Y%m%d)
Return-Path: GNMLendingDBACore@wellsfargo.com
MIME-Version: 1.0
...
Content-Dispo${COD}ion: attachment; filename="AWR_${COD}_${startSnap}_${endSnap}_REPORT.zip"
_EOF_
/usr/bin/uuencode -m ${TEMPDIR}/AWR_Report/AWR_${COD}_${startSnap}_${endSnap}_REPORT.zip AWR_${COD}_${startSnap}_${endSnap}_REPORT.zip
fi
if [[ ${ADDM} -eq 1 ]]
then
cat <<_EOF_
---$MAILPART
Content-type: application/zip; name="ADDM_${COD}_${startSnap}_${endSnap}_REPORT.zip"
Content-Transfer-Encoding: base64
Content-Dispo${COD}ion: attachment; filename="ADDM_${COD}_${startSnap}_${endSnap}_REPORT.zip"
_EOF_

/usr/bin/uuencode -m ${TEMPDIR}/ADDM_Report/ADDM_${COD}_${startSnap}_${endSnap}_REPORT.zip ADDM_${COD}_${startSnap}_${endSnap}_REPORT.zip

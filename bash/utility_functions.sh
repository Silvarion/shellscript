...
function continueQuestion {
        ...
        do
                if [[ $ANSWER != "NONE" ]]
                then
                        printf "[ATTENTION] Please enter \"y\" or \"n\" without the double quotes and press <ENTER>"
                        printf " "
                fi
                printf -n "[INPUT] Do you want to continue? (y/n) <ENTER> for [y]: "
                read ANSWER
                if [[ -z $ANSWER ]]
                then
                        ANSWER="y"
